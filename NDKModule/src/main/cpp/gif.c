...
openStream(JNIEnv *env, jobject stream) {
    ...
    container->
            buffer = (*env)->NewGlobalRef(env, bufferArray);
    if (container->buffer == NULL) {
        free(container);
        throwException(env, OUT_OF_MEMORY_ERROR,
                       OOME_MESSAGE);
    ...
    if (streamClass == NULL) {
        (*env)->
                DeleteGlobalRef(env, container
                ->buffer);
        free(container);
        throwException(env, RUNTIME_EXCEPTION_BARE,
                       "NewGlobalRef failed");
        return NULL_GIF_INFO;
    }
    ...
    container->
            readMethodID = (*env)->GetMethodID(env, streamClass, "read", "([BII)I");
    container->
            resetMethodID = (*env)->GetMethodID(env, streamClass, "reset", "()V");
    container->
            closeMethodID = (*env)->GetMethodID(env, streamClass, "close", "()V");
    ...
    container->
            stream = (*env)->NewGlobalRef(env, stream);
    ...
    container->
            markCalled = false;
    descriptor.
            GifFileIn = DGifOpen(container, &streamRead, &descriptor.Error);

    (*env)->
            CallVoidMethod(env, stream, markMethodID, INT32_MAX);
